...
`ep_inline_toolbar` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_inline_toolbar`:
...
* Server: `loadSettings`, `eejsBlock_body`, `eejsBlock_mySettings`, `eejsBlock_styles`, `clientVars`
* Client: `postAceInit`, `aceSelectionChanged`, `postToolbarInit`
